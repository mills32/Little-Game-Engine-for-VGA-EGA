...
void LT_Load_Animation(char *file, char *dat_string){
	...
	for (tileY=0;tileY<32;tileY+=32){
		for (tileX=0;tileX<width;tileX+=size){
			...
			for(x=0;x<sizey;x++){
				memcpy(&LT_tile_tempdata2[index],&LT_tile_tempdata[offset+(x<<w)],size);
				index+=size;
			}
		}
	}
	...
	if (LT_VIDEO_MODE == 4){
		memcpy(&LT_sprite_data[0],LT_tile_tempdata2,2048);
	}
...
void interrupt LT_Loading(void){
	asm CLI
	{
		...
		if (LT_VIDEO_MODE == 4) {
			...
			for(i = 0; i < 8;i++){
				memcpy(&CGA[offset1],&LT_sprite_data[offset],16);offset1+=8192;offset+=16;
				memcpy(&CGA[offset1],&LT_sprite_data[offset],16);offset1+=8192;offset+=16;
				memcpy(&CGA[offset1],&LT_sprite_data[offset],16);offset1+=8192;offset+=16;
				memcpy(&CGA[offset1],&LT_sprite_data[offset],16);offset1-=((8192*3)-160);offset+=16;
			}
		}
	}
	asm STI
...
void LT_Load_Image(char *file,char* dat_string){
	...
	asm CLI
	LT_Draw_Text_Box(11,18,16,1,0,"LOADING:  IMAGE ");
	asm STI
...
void LT_Load_Tiles(char *file,char* dat_string){
	...
	asm CLI
	LT_Draw_Text_Box(11,18,16,1,0,"LOADING: TILESET");
	asm STI
...
void LT_Load_Map(char *file, char* dat_string){ 
	...
	asm CLI
	LT_Draw_Text_Box(11,18,16,1,0,"LOADING: TMX MAP");
	asm STI
...
void LT_Scroll_Map_TGA(void){
	...
	if (LT_current_y != LT_last_y) LT_flipscrollv = 1;
	
	if (LT_flipscroll == 0){//Wait for columns to be drawn
		if (LT_flipscrollv == 1){
...
void LT_Cycle_Palette_TGA_EGA(byte *p,byte *c,byte speed){
	...
	if (LT_VIDEO_MODE == 0){
		for (x= 0; x<4;x++){
			...
			if (col > 7) col+=8;
			inportb(0x03DA);
			outportb(0x03C0,index);	//Index (0-15)
			outportb(0x03C0,col);	//Color
			outportb(0x03C0,0x20);
		}
	}
...
void LT_Fade_in_TGA(){
	...
	while (i < 6){
		for (j= 0; j<16;j++){
			col = LT_EGA_FADE[j+fade];
			outportb(0x03DA,0x10+j);	//Index (0-15)
			outportb(0x03DE,col);		//Color (0-15)
		}
		fade-=16;
		i++;
		LT_vsync();LT_vsync();
